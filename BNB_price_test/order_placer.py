...
class Account_bin:
    ...
    def __init__(self, binAcc):
        self.holding = False
        self.__bin_Acc = binAcc
        self.__bin_Acc.get_account_balances()
        self.usdt = self.__bin_Acc.usdt_balance
        self.bnbvalue = self.__bin_Acc.bnb_balance
        
        self.start_usdt = self.usdt
        
        self.prev_usdtbalance = self.usdt
        self.profit_loss = 0
    
    def Buy(self):
        self.__bin_Acc.place_buy_order_usdt(self.usdt)
        self.__bin_Acc.get_account_balances()
        self.prev_usdtbalance = self.usdt
        self.usdt = self.__bin_Acc.usdt_balance
        self.bnbvalue = self.__bin_Acc.bnb_balance
    ...
    def Sell(self):
        self.__bin_Acc.place_order('SELL', self.bnbvalue)
        self.__bin_Acc.get_account_balances()
        self.usdt = self.__bin_Acc.usdt_balance
        self.bnbvalue = self.__bin_Acc.bnb_balance
        self.holding = False
        
        self.profit_loss = self.prev_usdtbalance - self.usdt
        
bin_Acc = BinanceAccount()
real_test = Account_bin(bin_Acc)

while True:
    
    bin_Acc.get_account_balances()
    
    bin_Acc.update_data()
    bin_Acc.saveData("real_test_3.csv")
    bin_Acc.calculate_moving_averages()
    
    row = bin_Acc.current_data.iloc[-1]
        
    if row['MA_25'] == row['MA_25'] and row['MA_75'] == row['MA_75']:
        if(row['MA_25'] < row['MA_75']):
            print(row['MA_25'], row['MA_25'], row['MA_25'] - row['MA_75'])
            if real_test.holding:
                print("SELL CALL")
                real_test.Sell()
        else:
            if not real_test.holding:
                print("BUY CALL")
                real_test.Buy()
    ...
    print("----------------------------")
    print(f"DT: {current_datetime}")
    print(f"BNB Value: ", real_test.bnbvalue)
    print(f"USDT Value: ", real_test.usdt)
    print(f"P/L of Last trade: ", real_test.profit_loss)
    print(f"P/L Total: ", real_test.prev_usdtbalance - real_test.start_usdt)
    print("----------------------------")
    
    with open('output.csv', 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([current_datetime, real_test.bnbvalue, real_test.usdt])
